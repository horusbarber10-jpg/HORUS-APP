...
const EMAILJS_SERVICE     = "service_hw0edjg";
const EMAILJS_TPL_CLIENTE = "template_2lbjcad";
const EMAILJS_TPL_BARBERO = "template_7po1pzn";
...
const BONOS = [
  { id: 1, nombre: "Bono 5 Cortes",        precio: 55, ahorro: 10, emoji: "🎫", color: "#c9a84c", desc: "5 cortes al precio de 4,2" },
  { id: 2, nombre: "Bono 5 Corte+Cejas",   precio: 65, ahorro: 10, emoji: "🎫", color: "#e07b54", desc: "5 cortes + cejas" },
  { id: 3, nombre: "Bono 5 Corte+Barba",   precio: 75, ahorro: 10, emoji: "🎫", color: "#7b9e87", desc: "5 cortes + barba" },
  { id: 4, nombre: "Bono 5 Corte Premium", precio: 90, ahorro: 10, emoji: "🎫", color: "#5b8cd7", desc: "5 cortes premium" },
];
...
export default function HorusApp() {
  ...
  // Envía email usando la API REST de EmailJS — SIN paquete npm, build limpio en Vercel
  const sendEmailJS = (templateId, params) =>
    fetch("https://api.emailjs.com/api/v1.0/email/send", {
      ...
      .then((r) => r.ok ? console.log("✅ Email enviado:", templateId) : r.text().then((t) => console.error("❌ EmailJS:", t)))
      .catch((e) => console.error("❌ Error email:", e));

  // Envía DOS emails: al cliente y al barbero
  const enviarConfirmacion = (nombre, email, servicio, fecha, hora) => {
    const params = { cliente_nombre: nombre, cliente_email: email, servicio, fecha, hora, barberia_email: HORUS_EMAIL };
    sendEmailJS(EMAILJS_TPL_CLIENTE, params);
    sendEmailJS(EMAILJS_TPL_BARBERO, { ...params, to_email: HORUS_EMAIL });
  };
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text",  text: "Analiza la forma de la cara de esta persona y recomienda el mejor corte de pelo o estilo de barba. Sé específico y amigable." },
            ],
          }],
        }),
      });
      ...
      <div style={{ textAlign: "center", zIndex: 1, padding: 32, width: "100%" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => window.open(`https://wa.me/${WHATSAPP_NUMBER}`, "_blank")}
            style={{ flex: 1, padding: "10px", background: "rgba(37,211,102,0.1)", border: "1px solid rgba(37,211,102,0.2)", borderRadius: 10, color: "#25d366", fontSize: 12, cursor: "pointer" }}>
            💬 WhatsApp
          </button>
          <button onClick={() => window.open(`https://instagram.com/${INSTAGRAM}`, "_blank")}
            style={{ flex: 1, padding: "10px", background: "rgba(225,48,108,0.1)", border: "1px solid rgba(225,48,108,0.2)", borderRadius: 10, color: "#e1306c", fontSize: 12, cursor: "pointer" }}>
            📸 Instagram
          </button>
        </div>
  ...
  if (authStep === "login") return (
    <div style={{ ...base, justifyContent: "center", padding: "40px 28px" }}>
      ...
      <div style={{ zIndex: 1 }}>
        ...
        <div style={{ textAlign: "center", fontSize: 13, color: "rgba(240,236,228,0.4)", marginTop: 16 }}>
          ¿No tienes cuenta? <span onClick={() => setAuthStep("register")} style={{ color: "#c9a84c", cursor: "pointer" }}>Regístrate</span>
        </div>
      </div>
    </div>
  );
  ...
  if (authStep === "register") return (
    <div style={{ ...base, padding: "40px 28px", overflowY: "auto" }}>
      ...
      <div style={{ zIndex: 1 }}>
        ...
        <div style={{ textAlign: "center", fontSize: 13, color: "rgba(240,236,228,0.4)", marginTop: 16 }}>
          ¿Ya tienes cuenta? <span onClick={() => setAuthStep("login")} style={{ color: "#c9a84c", cursor: "pointer" }}>Inicia sesión</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ display: "flex", padding: "6px 10px", gap: 3, background: "rgba(10,10,10,0.95)", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
        {[["home","🏠","Inicio"],["reservar","📅","Reservar"],["galeria","📸","Galería"],["estilo","🎨","Estilo IA"],["chat","💬","Chat"],["miscitas","✂️","Mis Citas"]].map(([tab, icon, label]) => (
          <button key={tab} onClick={() => setScreen(tab)}
            style={{ flex: 1, padding: "6px 2px", background: screen === tab ? "rgba(201,168,76,0.12)" : "none", border: screen === tab ? "1px solid rgba(201,168,76,0.2)" : "1px solid transparent", borderRadius: 8, color: screen === tab ? "#c9a84c" : "rgba(240,236,228,0.35)", fontSize: 9, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px 80px" }}>
        ...
        {screen === "home" && (
          <div className="fade-in">
            ...
            {SERVICIOS.map((s) => (
              <div key={s.id} className="glass hover-scale" onClick={() => { setSelectedServicio(s); setScreen("reservar"); }}
                style={{ borderRadius: 12, padding: "11px 13px", marginBottom: 8, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {BONOS.map((b) => (
              <div key={b.id} className="glass hover-scale" onClick={() => window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=Quiero comprar ${b.nombre}`, "_blank")}
                style={{ borderRadius: 12, padding: "11px 13px", marginBottom: 8, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
        ...
        {screen === "reservar" && (
          <div className="fade-in">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 14 }}>
              {SERVICIOS.map((s) => (
                <div key={s.id} onClick={() => setSelectedServicio(s)}
                  style={{ borderRadius: 12, padding: 12, border: `1px solid ${selectedServicio?.id === s.id ? s.color : "rgba(255,255,255,0.07)"}`, background: selectedServicio?.id === s.id ? `${s.color}15` : "rgba(255,255,255,0.03)", cursor: "pointer", textAlign: "center" }}>
            ...
            <button
              onClick={async () => {
                ...
                const nuevaCita = {
                  ...
                  precio:   selectedServicio.promo || selectedServicio.precio,
                  fecha:    selectedFecha,
              ...
              style={{ width: "100%", padding: "14px", fontSize: 13, borderRadius: 14, background: "linear-gradient(135deg,#25d366,#1ebe5d)", border: "none", color: "#fff", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              💬 Confirmar por WhatsApp
            </button>
            <div style={{ textAlign: "center", fontSize: 9, color: "rgba(240,236,228,0.2)", marginTop: 8 }}>
              Se guardará en la base de datos · Email automático · Se abrirá WhatsApp
            </div>
          </div>
        )}
        ...
        {screen === "galeria" && (
          <div className="fade-in">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
              {GALERIA.map((g) => (
                <div key={g.id} onClick={() => window.open(g.url, "_blank")}
                  style={{ borderRadius: 12, overflow: "hidden", position: "relative", cursor: "pointer", transition: "transform 0.2s" }}
            ...
            <button onClick={() => window.open(`https://instagram.com/${INSTAGRAM}`, "_blank")}
              style={{ width: "100%", padding: "11px", background: "rgba(225,48,108,0.1)", border: "1px solid rgba(225,48,108,0.2)", borderRadius: 12, color: "#e1306c", fontSize: 12, cursor: "pointer", marginBottom: 10 }}>
            ...
            {bookings.length === 0 ? (
              ...
              <div key={i} className="glass" style={{ borderRadius: 14, padding: 13, marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => enviarWhatsApp(b.servicio, b.hora, b.fecha, user?.name)}
                    style={{ flex: 1, padding: "8px", background: "rgba(37,211,102,0.1)", border: "1px solid rgba(37,211,102,0.15)", borderRadius: 9, color: "#25d366", fontSize: 10, cursor: "pointer" }}>💬 WhatsApp</button>
                  <button onClick={() => enviarRecordatorio(b)}
                    style={{ flex: 1, padding: "8px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 9, color: "rgba(240,236,228,0.5)", fontSize: 10, cursor: "pointer" }}>⏰ Recordatorio</button>
                </div>
              </div>
            ))}
            {bookings.length > 0 && (
              <div style={{ marginTop: 14 }}>
                ...
                <div className="glass" style={{ borderRadius: 13, padding: 13 }}>
                  ...
                  <textarea value={nuevaResena.texto} onChange={(e) => setNuevaResena((r) => ({ ...r, texto: e.target.value }))}
          ...
          <div className="fade-in">
            ...
            <div className="glass" style={{ borderRadius: 14, padding: 13, marginBottom: 12 }}>
              ...
              <button className="btn-gold" onClick={async () => {
                ...
                await db.post("reservas", { ...nuevaCita, email: "manual", nombre: adminForm.nombre });
                setBookings((p) => [...p, nuevaCita]);
            ...
            <div className="glass" style={{ borderRadius: 14, padding: 13, marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {[0, 15, 20, 30, 45, 60].map((t) => (
                  <button key={t} onClick={() => setTiempoEspera(t)}
                    style={{ padding: "7px 11px", borderRadius: 8, border: `1px solid ${tiempoEspera === t ? "#c9a84c" : "rgba(255,255,255,0.08)"}`, background: tiempoEspera === t ? "rgba(201,168,76,0.12)" : "rgba(255,255,255,0.04)", color: tiempoEspera === t ? "#c9a84c" : "rgba(240,236,228,0.5)", fontSize: 11, cursor: "pointer" }}>
            ...
            <div className="glass" style={{ borderRadius: 14, padding: 13, marginBottom: 12 }}>
              ...
              <input type="date"
                style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "7px 10px", color: "#f0ece4", fontSize: 12, marginBottom: 9 }}
                onChange={(e) => {
            ...
            <div className="glass" style={{ borderRadius: 14, padding: 13, marginBottom: 12 }}>
              ...
              {bookings.length === 0 ? (
                ...
                <div key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", paddingBottom: 10, marginBottom: 10 }}>
                  ...
                  <button onClick={() => enviarRecordatorio(b)}
                    style={{ marginTop: 5, padding: "5px 10px", background: "rgba(255,193,7,0.1)", border: "1px solid rgba(255,193,7,0.2)", borderRadius: 7, color: "#ffc107", fontSize: 10, cursor: "pointer" }}>
...
function Spinner() {
  return <span style={{ display: "inline-block", width: 15, height: 15, border: "2px solid rgba(10,10,10,0.3)", borderTopColor: "#0a0a0a", borderRadius: "50%", animation: "spin 0.6s linear infinite" }} />;
}
