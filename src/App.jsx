...
Servicios: Corte 13€ (30min), Corte+Cejas 15€ (40min), Corte+Barba 17€ (50min), Corte Premium 20€ rebajado a 17€ (60min).
...
Responde en español, amigable y profesional. Ayuda con reservas, precios, horarios, consejos de estilo y recomendaciones de corte según tipo de cara.`;
...
export default function HorusApp() {
  ...
  useEffect(() => {
    if (selectedFecha) {
      db.get("reservas", `fecha=eq.${selectedFecha}`).then(data => setHorariosOcupados(data.map(r => r.hora)));
    }
  }, [selectedFecha]);
  ...
  const analizarEstilo = async (base64) => {
    ...
    } catch { setRecomendacion("Error al analizar. Inténtalo de nuevo."); }
    setAnalizando(false);
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:600, system:CHAT_SYSTEM,
  ...
  const servicioMasPedido = bookings.length ? bookings.reduce((acc,b)=>{ acc[b.servicio]=(acc[b.servicio]||0)+1; return acc; },{}) : {};
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 14px 80px"}}>
        ...
        {screen==="reservar"&&(
          <div className="fade-in">
              ...
              <>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:5,marginBottom:18}}>
                  {HORARIOS_BASE.map(h=>{
                    ...
                    return <button key={h} onClick={()=>!ocupado&&setSelectedHorario(h)} style={{padding:"8px 2px",borderRadius:9,border:"none",cursor:ocupado?"not-allowed":"pointer",fontSize:10,fontWeight:500,background:ocupado?"rgba(255,255,255,0.02)":selectedHorario===h?"linear-gradient(135deg,#c9a84c,#e8c97a)":"rgba(255,255,255,0.06)",color:ocupado?"rgba(240,236,228,0.15)":selectedHorario===h?"#0a0a0a":"#f0ece4",textDecoration:ocupado?"line-through":"none"}}>{h}</button>;
                  })}
                </div>
              </>
            )}
            ...
            <button onClick={async()=>{
              ...
              const nuevaCita={servicio:selectedServicio.nombre,precio:selectedServicio.promo||selectedServicio.precio,fecha:selectedFecha,hora:selectedHorario};
              await db.post("reservas",{nombre,email:user?.email||"",servicio:selectedServicio.nombre,precio:selectedServicio.promo||selectedServicio.precio,fecha:selectedFecha,hora:selectedHorario});
              ...
              addNotification("📅 ¡Cita reservada!",`${selectedServicio.nombre} el ${selectedFecha} a las ${selectedHorario}. Recordatorio 24h antes.`);
              setScreen("miscitas");
            }} style={{width:"100%",padding:"14px",fontSize:13,borderRadius:14,background:"linear-gradient(135deg,#25d366,#128c7e)",border:"none",color:"white",cursor:"pointer",fontWeight:700,boxShadow:"0 8px 20px rgba(37,211,102,0.2)"}}>
              💬 Confirmar por WhatsApp
            </button>
            <div style={{textAlign:"center",fontSize:9,color:"rgba(240,236,228,0.2)",marginTop:6}}>Se abrirá WhatsApp con tu reserva lista</div>
          </div>
        )}
        ...
        {screen==="chat"&&(
          <div className="fade-in" style={{display:"flex",flexDirection:"column",height:"calc(100vh - 195px)"}}>
            ...
            <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:8,paddingBottom:8}}>
              ...
              {isTyping&&<div style={{display:"flex",gap:4,padding:"9px 13px",background:"rgba(255,255,255,0.07)",borderRadius:"14px 14px 14px 4px",width:"fit-content"}}>
            ...
            <div style={{display:"flex",gap:6,paddingTop:8,borderTop:"1px solid rgba(255,255,255,0.05)"}}>
              <input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMessage()} placeholder="Precios, horarios, estilos..." style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.09)",borderRadius:10,padding:"10px 12px",color:"#f0ece4",fontSize:12,outline:"none"}}/>
        ...
        {screen==="resenas"&&(
          <div className="fade-in">
            ...
            <div style={{fontSize:11,color:"rgba(240,236,228,0.35)",marginBottom:14}}>{resenas.length} reseñas · {(resenas.reduce((a,r)=>a+r.estrellas,0)/resenas.length).toFixed(1)} promedio</div>
            {resenas.map(r=>(
              <div key={r.id} className="glass" style={{borderRadius:12,padding:"11px 13px",marginBottom:7}}>
